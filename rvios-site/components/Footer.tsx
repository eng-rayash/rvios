...
export default function Footer() {
  ...
  return (
    <footer className="bg-surface-2 text-ivory">
      {/* Main Footer */}
          ...
          <div className="md:col-span-4">
            <Link href="/" className="flex items-center gap-3 group mb-5">
              ...
              <div className="flex flex-col leading-none">
                ...
                <span className="font-body text-[0.6rem] tracking-[0.22em] text-fg-muted uppercase">Technologies</span>
              </div>
            </Link>
            <p className="font-body text-sm leading-relaxed text-fg-muted max-w-xs">
              حلول تقنية متكاملة لبناء أعمال رقمية أكثر ذكاءً.
            </p>
            <p className="font-body text-xs tracking-[0.15em] text-fg-muted mt-1 uppercase">
            ...
            <div className="mt-6 space-y-3">
              <a
                ...
                className="flex items-center gap-3 font-body text-sm text-fg-muted hover:text-ivory transition-colors group"
              >
                <span className="w-8 h-8 rounded-full bg-surface-3 flex items-center justify-center group-hover:bg-primary/30 transition-colors shrink-0">
              ...
              <a
                ...
                className="flex items-center gap-3 font-body text-sm text-fg-muted hover:text-ivory transition-colors group"
              >
                <span className="w-8 h-8 rounded-full bg-surface-3 flex items-center justify-center group-hover:bg-green-900/50 transition-colors shrink-0">
            ...
            <ul className="space-y-3">
              {SERVICES_LINKS.map((s) => (
                <li key={s.href}>
                  ...
                  >
                    <span className="font-body text-sm text-fg-muted group-hover:text-ivory transition-colors">{s.ar}</span>
                    <span className="font-body text-[0.65rem] tracking-[0.15em] text-fg-muted uppercase">{s.en}</span>
                  </Link>
                </li>
              ))}
            </ul>
              ...
              {COMPANY_LINKS.map((c) => (
                <li key={c.href}>
                  <Link
                    ...
                    className="group flex flex-col hover:translate-x-[-4px] transition-transform duration-200"
                  >
                    <span className="font-body text-sm text-fg-muted group-hover:text-ivory transition-colors">{c.ar}</span>
                    <span className="font-body text-[0.65rem] tracking-[0.15em] text-fg-muted uppercase">{c.en}</span>
                  </Link>
                </li>
              ))}
            </ul>
      ...
      <div className="border-t border-border-subtle">
        <div className="mx-auto max-w-7xl px-6 py-5 flex flex-col items-center justify-between gap-3 md:flex-row">
          ...
          <div className="flex items-center gap-4">
            <a
              href="/rayash"
              className="font-body text-xs text-fg-muted hover:text-fg-muted transition-colors"
            >
              Created by{" "}
            ...
            <span className="font-body text-xs tracking-[0.15em] text-fg-muted uppercase">
              rvios.com
            </span>
          </div>
        </div>
